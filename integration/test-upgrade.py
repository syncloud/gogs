...
def test_upgrade(device, device_user, device_password, device_host, app_archive_path, app_domain):
    ...
    check_output("git clone https://{0}:{1}@{2}/{3}/init init".format(device_user, device_password, app_domain, device_user), shell=True)
    check_output("cd init; touch 1; git add .; git commit -am 'test'; git push;", shell=True)
